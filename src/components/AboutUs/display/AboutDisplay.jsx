...
export default function AboutDisplay() {
  ...
  return (
    <div
      ref={ref}
      className="w-full py-10 md:py-0 md:h-[90vh] bg-gradient-to-b from-blue-950 via-blue-950 to-blue-900 flex items-center justify-center overflow-hidden"
    >
      <div className="w-full h-full flex items-center justify-center bg-[url(/aboutus/about2bgB.png)] bg-no-repeat bg-bottom">
        <div className="relative w-[70%] lg:w-[60%] max-w-7xl md:h-[70%]">
          {/* Circle Container */}
          {data.map((info, index) => {
            const baseClass = `md:absolute mb-5 md:mb-0 w-[300px] h-[300px] lg:w-[400px] lg:h-[400px] rounded-full 
             from-blue-900 via-blue-900 to-purple-400 
            flex items-center justify-center flex-col text-white shadow-2xl`;

            const positionClass =
              index === 0
                ? "md:left-8 md:bottom-12"
                : index === 1
                  ? "md:left-1/2 md:-translate-x-1/2 top-0"
                  : "md:right-8 md:bottom-12";
            ...
            return (
              <motion.div
                ...
                className={`${baseClass} ${index == 1 ? "bg-gradient-to-bl" : "bg-gradient-to-tl"
                  } ${positionClass}`}
              >
